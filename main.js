...
function loadMessagesIntoChannel() {
    for(var i;i<mockChannels.length;i++){ 
        for (var j = 0; i < mockMessages.length; i++) {
           if(mockChannels[i].id==mockMessages[j].channel){
            mockChannels[i].messages.push(mockMessages[j].text);
           }
        }   
    }
...
function displayChannels() {
    var regularCh=document.getElementById("regular-channels");
    for (var i = 0; i < mockChannels.length;i++){
    var list= document.createElement("li");
    list.innerHTML=`<i class="material-icons"> group </i>
          <span class="channel-name"> ${mockChannels[i].name} </span>
          <span class="timestamp">${mockChannels[i].latestMessage}</span>
        </li>`;
        regularCh.appendChild(list);
